...
$meta_box_partner_images = array(
	'id' => 'mega-meta-box-partner-images',
	'title' =>  __( 'Services Gallery', 'mega' ),
	'page' => 'partner',
	'context' => 'normal',
	'priority' => 'high',
	'fields' => array(
...
add_action('add_meta_boxes', 'mega_add_box_partner');

function mega_add_box_partner() {
	global $meta_box_partner_images;
	add_meta_box($meta_box_partner_images['id'], $meta_box_partner_images['title'], 'mega_show_box_partner_images', $meta_box_partner_images['page'], $meta_box_partner_images['context'], $meta_box_partner_images['priority']);
}
 ...
 */
function mega_show_box_partner_images() {
	global $meta_box_partner_images, $post;
		...
		<tr style="border-top:1px solid #eeeeee;">
			<td>				
				<div id="gallery_images_container">
					<ul class="gallery_images">
						<?php
							if ( metadata_exists( 'post', $post->ID, '_partner_image_gallery' ) ) {
								$post_image_gallery = get_post_meta( $post->ID, '_partner_image_gallery', true );
							} else {
								// Backwards compat
					...
					<input type="hidden" id="partner_image_gallery" name="partner_image_gallery" value="<?php echo esc_attr( $post_image_gallery ); ?>" />
				</div>
				...
				<script type="text/javascript">
					jQuery(document).ready(function($){
						...
						var $image_gallery_ids = $('#partner_image_gallery');
 ...
 */
add_action( 'save_post', 'mega_save_data_partner' );
 
function mega_save_data_partner($post_id) {
	global $meta_box_partner_images;
	...
	if ( isset($_POST['mega_meta_box_nonce'])) {
		...
		$attachment_ids = array_filter( explode( ',', mega_clean( $_POST['partner_image_gallery'] ) ) );
		update_post_meta( $post_id, '_partner_image_gallery', implode( ',', $attachment_ids ) );
