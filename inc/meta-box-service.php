...
$meta_box_service_images = array(
	'id' => 'mega-meta-box-service-images',
...
add_action('add_meta_boxes', 'mega_add_box_service');

function mega_add_box_service() {
	global $meta_box_service_images;
	add_meta_box($meta_box_service_images['id'], $meta_box_service_images['title'], 'mega_show_box_service_images', $meta_box_service_images['page'], $meta_box_service_images['context'], $meta_box_service_images['priority']);
}
 ...
 */
function mega_show_box_service_images() {
	global $meta_box_service_images, $post;
		...
		<tr style="border-top:1px solid #eeeeee;">
			<td>				
				<div id="gallery_images_container">
					<ul class="gallery_images">
						<?php
							if ( metadata_exists( 'post', $post->ID, '_service_image_gallery' ) ) {
								$post_image_gallery = get_post_meta( $post->ID, '_service_image_gallery', true );
							} else {
								// Backwards compat
					...
					<input type="hidden" id="service_image_gallery" name="service_image_gallery" value="<?php echo esc_attr( $post_image_gallery ); ?>" />
				</div>
				...
				<script type="text/javascript">
					jQuery(document).ready(function($){
						...
						var $image_gallery_ids = $('#service_image_gallery');
 ...
 */
add_action( 'save_post', 'mega_save_data_service' );
 
function mega_save_data_service($post_id) {
	global $meta_box_service_images;
	...
	if ( isset($_POST['mega_meta_box_nonce'])) {
		...
		$attachment_ids = array_filter( explode( ',', mega_clean( $_POST['service_image_gallery'] ) ) );
		update_post_meta( $post_id, '_service_image_gallery', implode( ',', $attachment_ids ) );
