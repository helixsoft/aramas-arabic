...
$prefix = 'mega_';
$meta_box_promotion_images = array(
	'id' => 'mega-meta-box-promotion-images',
	'title' =>  __( 'Promotion Gallery', 'mega' ),
	'page' => 'promotion',
	'context' => 'normal',
	'priority' => 'high',
	'fields' => array(
...
add_action('add_meta_boxes', 'mega_add_box_promotion');

function mega_add_box_promotion() {
	global $meta_box_promotion_images;
	add_meta_box($meta_box_promotion_images['id'], $meta_box_promotion_images['title'], 'mega_show_box_promotion_images', $meta_box_promotion_images['page'], $meta_box_promotion_images['context'], $meta_box_promotion_images['priority']);
}
 ...
 */
function mega_show_box_promotion_images() {
	global $meta_box_promotion_images, $post;
			...
			<td>				
				<div id="gallery_images_container">
					<ul class="gallery_images">
						<?php
							if ( metadata_exists( 'post', $post->ID, '_promotion_image_gallery' ) ) {
								$post_image_gallery = get_post_meta( $post->ID, '_promotion_image_gallery', true );
							} else {
								// Backwards compat
					...
					<input type="hidden" id="promotion_image_gallery" name="promotion_image_gallery" value="<?php echo esc_attr( $post_image_gallery ); ?>" />
				</div>
				...
				<script type="text/javascript">
					jQuery(document).ready(function($){
						...
						var $image_gallery_ids = $('#promotion_image_gallery');
 ...
 */
add_action( 'save_post', 'mega_save_data_promotion' );
 
function mega_save_data_promotion($post_id) {
	global $meta_box_promotion_images;
	...
	if ( isset($_POST['mega_meta_box_nonce'])) {
		...
		$attachment_ids = array_filter( explode( ',', mega_clean( $_POST['promotion_image_gallery'] ) ) );
		update_post_meta( $post_id, '_promotion_image_gallery', implode( ',', $attachment_ids ) );
